...
export default {
  ...
  mounted () {
    ...
    this.initLineChart()
    this.initMap()
    // this.initSocket() // ✅ 连接后端socket
  },
  // ==============================================
  // 【新增 2/3】切回路由时，重新激活 GPS 监听
  // ==============================================
  activated () {
    if (this.$socket) {
      this.$socket.on('gps', this.$options.sockets.gps.bind(this))
    }
  },
  beforeDestroy () {
  ...
  methods: {
    // ==============================================
    // 【新增 3/3】把原来的 gps 方法提取出来，方便复用
    // ==============================================
    handleGps (data) {
      this.$options.sockets.gps.call(this, data)
    },
    getHeightsWidths () {
